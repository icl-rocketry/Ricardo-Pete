...
void DPS310::setup()
{
    ...
    Serial.println("DPS310 chillin");
    
    if (m_productID != 0 || m_revisionID != 1)
    {
        Serial.println("DPS310 failed to respond with expected prod and rev ID, Prod Id:" + String(m_productID) + ", Rev Id: " + String(m_revisionID));
...
void DPS310::update()
{
    ...
    readDPS();
    baro_temperature += 273.15;
    baro_altitude = toAltitude(baro_pressure);
}
...
void DPS310::readDPS()
{
    ...
    baro_temperature = calcTemp(raw_temp);
    baro_pressure = calcPressure(raw_press);
}
